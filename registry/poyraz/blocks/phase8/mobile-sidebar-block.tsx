...
import { Sidebar, SidebarContent, SidebarGroup, SidebarGroupLabel, SidebarHeader, SidebarMenu, SidebarMenuItem } from "@/components/ui/organisms/sidebar";

function MobileSidebarBlock() {
  return (
    <div data-slot="mobile-sidebar-block">
    <Sheet>
      <SheetTrigger asChild><Button variant="outline" size="icon" aria-label="Open workspace navigation"><Menu className="size-4" /></Button></SheetTrigger>
      <SheetContent side="left" surface="glass" className="w-[min(88vw,20rem)] p-0" showClose>
        ...
        <Sidebar variant="default" className="h-full w-full border-0">
          ...
          <SidebarContent><SidebarGroup><SidebarGroupLabel>Navigation</SidebarGroupLabel><SidebarMenu>
            <SidebarMenuItem active icon={<Home className="size-4" />}>Overview</SidebarMenuItem>
