  ...
  "poyraz-card relative flex flex-col overflow-hidden border text-card-foreground transition-[color,background-color,border-color,transform] duration-[var(--poyraz-motion-duration-base)] ease-[var(--poyraz-motion-ease-out)]",
  {
    variants: {
      variant: {
        ...
        glass:
          "border-glass-border-outer bg-glass shadow-[var(--poyraz-glass-shadow)] backdrop-blur-glass",
        ...
        ghost:
          "border-transparent bg-transparent shadow-none hover:border-border hover:bg-surface-subtle",
      },
      radius: {
        none: "rounded-none",
        md: "rounded-md",
...
const CardImage = React.forwardRef<HTMLDivElement, CardImageProps>(
  ({ className, aspect = "aspect-video", children, ...props }, ref) => (
    <div
      ...
      className={cn("relative overflow-hidden", "border-b border-border-strong", aspect, className)}
      {...props}
    >
      {children}
    </div>
...
const CardHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ...
      className={cn(
        "grid grid-cols-[1fr_auto] items-start gap-1.5 p-5 [&_[data-slot=card-description]]:col-start-1",
        className,
      )}
      {...props}
    />
...
const CardFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ...
      className={cn("mt-auto flex items-center gap-3 border-t border-border px-5 py-4", className)}
      {...props}
    />
  ),
);
