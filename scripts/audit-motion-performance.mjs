...
for (const root of ["components", "src"]) {
  for (const file of await files(root)) {
    ...
    if (/transition[^\n"']*box-shadow|transition-\[[^\]]*box-shadow/.test(source))
      findings.shadow.push(path);
    if (/transition[^\n"']*border-color|transition-\[[^\]]*border-color/.test(source))
      findings.borderColor.push(path);
    const keyframes = source.match(/@keyframes[^{}]+\{(?:[^{}]|\{[^{}]*\})*\}/g) ?? [];
    if (keyframes.some((block) => /(?:backdrop-filter|\bfilter\s*:)/.test(block)))
      findings.animatedBlur.push(path);
  }
}
...
console.log(
  `- border-color transition candidates: ${findings.borderColor.length} (color-only, no geometry)`,
);
...
if (findings.animatedBlur.length || findings.transitionAll.length || findings.shadow.length) {
  console.error(
    "Motion performance audit failed: broad transitions, shadow interpolation or animated blur/filter found.",
  );
  process.exitCode = 1;
}
