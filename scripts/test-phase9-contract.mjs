...
for (const name of ["micro", "floating"])
  if (!tokens.shared.motion.distance[name]) failures.push(`motion token: distance.${name} missing`);
for (const name of ["enter", "pressSmall", "pressMedium", "pressLarge"])
  if (!tokens.shared.motion.scale[name]) failures.push(`motion token: scale.${name} missing`);
if (Number.parseInt(tokens.shared.motion.duration.fast, 10) >= 200)
  failures.push("motion token: fast feedback must stay below 200ms");
...
for (const guard of [
  ...
  '[data-poyraz-motion="reduced"]',
  "animation-iteration-count: 1 !important",
])
  requireText("reduced motion", preset, guard);
const keyframes = preset.match(/@keyframes[^{}]+\{(?:[^{}]|\{[^{}]*\})*\}/g) ?? [];
if (keyframes.some((block) => /(?:backdrop-filter|\bfilter\s*:)/.test(block)))
  failures.push("performance: blur/filter is animated inside a keyframe");
...
for (const recipe of [
  ...
  "accordionMotion",
  "toastMotion",
  "animate-poyraz-floating-in",
  "animate-poyraz-overlay-in",
])
  requireText("motion recipes", recipes, recipe);
for (const scale of ["press-small", "press-medium", "press-large"])
  requireText("button size press scale", button, scale);
requireText(
  "accordion exit",
  await read("components/ui/molecules/accordion.tsx"),
  "accordionMotion",
);
requireText("toast swipe", await read("components/ui/molecules/sonner.tsx"), "toastMotion");
requireText(
  "motion docs",
  await read("docs/v3/phase-9-motion-system.md"),
...
for (const rule of [
  "@utility animate-poyraz-floating-in",
  "@utility animate-poyraz-spin",
