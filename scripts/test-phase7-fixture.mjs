...
for (const name of phase7) {
  const fixture = await mkdtemp(resolve(tmpdir(), `poyraz-phase7-${name}-`));
  for (const dependency of collect(name)) {
...
console.log(`Phase 7 standalone fixture typechecks passed (${phase7.length} items).`);
