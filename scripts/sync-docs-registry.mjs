...
function docsLocation(item) {
  if (item.type === "registry:block")
    return { section: "Blocks", href: `/docs/blocks/${item.name}` };
  if (hidden.has(item.name)) return null;
  if (organismNames.has(item.name))
    return { section: "Organisms", href: `/docs/organisms/${item.name}` };
  ...
  if (item.type === "registry:ui")
    return { section: "Atoms", href: `/docs/atoms/${aliases[item.name] ?? item.name}` };
  return null;
}
...
for (const item of items) {
  if (!unique.has(item.href) || item.name === item.href.split("/").at(-1))
...
const catalog = {
  ...
  blocks: items
    .filter(({ section }) => section === "Blocks")
    .sort((a, b) => a.title.localeCompare(b.title)),
};

await writeFile(resolve("src/docs-registry.json"), `${JSON.stringify(catalog, null, 2)}\n`);
console.log(
  `Docs registry synchronized (${catalog.counts.components} components, ${catalog.counts.blocks} blocks).`,
);
