...
for (const announcement of ['role="alert"', 'aria-live="assertive"', 'aria-atomic="true"']) requireText("form invalid announcement", form, announcement);

const calendar = await read("components/ui/molecules/calendar.tsx");
for (const state of ["data-range-start", "data-range-middle", "data-range-end", "data-today", 'mode: "range"']) requireText("calendar semantic state", calendar, state);
const datePicker = await read("components/ui/molecules/date-picker.tsx");
for (const api of ["defaultSelected", "defaultOpen", "selectionControlled", "openControlled", "closeOnSelect", "clearable"]) requireText("date picker control API", datePicker, api);

const autocomplete = await read("components/ui/molecules/autocomplete.tsx");
for (const keyboard of ['e.key === "Home"', 'e.key === "End"', 'e.key === "Escape"', 'e.key === "Tab"', "findEnabled"]) requireText("autocomplete keyboard", autocomplete, keyboard);
for (const aria of ["aria-activedescendant", "aria-controls", 'role="status"', 'role="alert"']) requireText("autocomplete state semantics", autocomplete, aria);

const dataCore = await read("components/ui/organisms/data-table-core.tsx");
for (const state of ['"populated"', '"empty"', '"loading"', '"error"']) requireText("data table state", dataCore, state);
requireText("sticky header", dataCore, "sticky top-0");
requireText("mobile overflow", dataCore, "overflow-auto");
if (/(tanstack|ag-grid|react-table)/i.test(dataCore)) failures.push("data-table-core: external data-grid dependency found");
requireText("data table composition", await read("components/ui/organisms/data-table.tsx"), "<DataTableCore");
...
const blockNames = ["article-card", "image-card", "news-card", "stats-card", "testimonial-card", "pricing-card", "product-card"];
...
for (const name of ["alert", "sonner", "form", "calendar", "date-picker", "autocomplete", "data-table-core", "data-table", "mermaid", "star-rating", ...blockNames]) {
  if (!registryItems.has(name)) failures.push(`registry: missing ${name}`);
}
for (const name of blockNames) if (registryItems.get(name)?.type !== "registry:block") failures.push(`registry: ${name} is not a block`);
if (!registryItems.get("form")?.meta?.optionalInstall) failures.push("registry: form is not marked optional install");

for (const path of ["app/docs/molecules/autocomplete/page.tsx", "app/docs/organisms/data-table/page.tsx"]) {
  const docs = await read(path);
  for (const state of ["loading", "empty", "error"]) requireText(`state docs ${path}`, docs.toLowerCase(), state);
}
...
console.log("Phase 7 contract is valid (feedback, selection, data, Mermaid, blocks and registry graph)." );
