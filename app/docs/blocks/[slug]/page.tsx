...
export default async function BlockPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const componentName = block.title
    .replace(/[^a-zA-Z0-9]+(.)/g, (_, character: string) => character.toUpperCase())
    .replace(/\s/g, "");
  const importName =
    block.name === "footer-blocks"
  ...
  const usageCode =
    block.name === "footer-blocks" ? "<BrandedFooterBlock />" : `<${componentName} />`;

  return (
    <ComponentPage name={block.title} description={block.description} importCode={importCode}>
      <DemoSection
        title="Live preview"
        description="Resize the block canvas independently from the documentation viewport."
        code={usageCode}
      >
        <BlockPreview>
