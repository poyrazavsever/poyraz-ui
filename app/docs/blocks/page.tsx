    ...
    <div className="space-y-10 pb-16">
      <header className="space-y-3">
        ...
        <p className="max-w-3xl text-muted-foreground">
          Source-owned layouts assembled from Poyraz primitives. Install a block, then edit its
          markup and data directly in your application.
        </p>
      </header>
        ...
        <div className="grid gap-px overflow-hidden rounded-md border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
          {previews.map((preview) => (
            <Link
              key={preview.href}
              href={preview.href}
              className="flex items-center justify-between bg-surface p-4 no-underline hover:bg-surface-subtle"
            >
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-poyraz-title font-semibold">
            All blocks ({docsCatalog.counts.blocks})
          </h2>
        </div>
        <div className="grid gap-4 lg:grid-cols-2">
          {docsCatalog.blocks.map((block) => (
            <Link
              key={block.name}
              href={block.href}
              className="group block rounded-md border border-border bg-surface p-4 no-underline transition-colors hover:border-border-strong hover:bg-surface-subtle"
            >
              <div className="mb-4">
              ...
              <div className="mt-3 flex items-center justify-between gap-3">
                <p className="text-xs text-placeholder">
                  {block.registryDependencies.length} registry dependencies / {block.files.length}{" "}
                  source files
                </p>
