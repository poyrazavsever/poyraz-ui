  ...
  return (
    <div className="space-y-10 pb-16">
      <header className="space-y-3">
        ...
        <p className="max-w-3xl text-muted-foreground">V2 package imports remain documented at this stable URL for existing applications. New applications should use the V3 registry.</p>
      </header>
...
@import "poyraz-ui/preset.css";`} />
      ...
      <section className="space-y-3">
        ...
        <CodeBlock code={`import { Button, Card, Input } from "poyraz-ui/atoms";
import { Dialog, Select } from "poyraz-ui/molecules";
import { Navbar, Sidebar } from "poyraz-ui/organisms";`} />
      </section>
      <Link href="/docs/migration" className="inline-flex items-center gap-2 text-sm font-semibold text-primary no-underline">Start the V3 migration <ArrowRight className="size-4" /></Link>
    </div>
  );
}
